...
class UploadedTools{
    ...
    public function Uploaded($file,$prefix = 'upload_',$filename){
        ...
        $date = date('Ymd');
        if(!is_dir($this->upload_dir.$filename."/".$date)){
            //文件不存在则创建
            mkdir($this->upload_dir.$filename."/".$date,0777,true);
        }
        if(move_uploaded_file($file["tmp_name"],$this->upload_dir.$filename."/".$date."/".$file_name)){
            #上传文件成功
            return $filename.'/'.$date."/".$file_name;  
        }else{
            $this->error_info = "图片上传失败！";
            return false;
        }
    }
}