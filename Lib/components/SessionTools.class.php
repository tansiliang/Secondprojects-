<?php
class SessionTools extends Model{
    public $db;
    public $pric= "wx_sesstion";
    /**
     * 类加载时调用session入库的各项
     */
    ...
    public function sess_read($sess_id) {
		$sql = "select sess_data from `$this->pric` where sess_name='$sess_id'";
    ...
    public function sess_write($sess_id, $sess_data) {
		$expire = time();
		$sql = "insert into `$this->pric` values ('$sess_id', '$sess_data', $expire) on duplicate key update sess_data='$sess_data', expact=$expire";
    ...
    public function sess_destroy($sess_id) {
		$sql = "delete from `$this->pric` where sess_name='$sess_id'";
		return $this->db->execute($sql);
	}
    /**
     * 在session_static()时执行
    ...
    public function sess_gc($ttl) {
		$last_time = time()-$ttl;
		$sql = "delete from `$this->pric` where expact < $last_time";
		return $this->db->execute($sql);
	}
